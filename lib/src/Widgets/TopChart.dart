...
class PercentOfDomainBarChart extends StatelessWidget {
  ...
  PercentOfDomainBarChart({ this.animate});
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<TopBarChart>(
      ...
      charts.Series<TopBarData, String>(
        ...
        data:[TopBarData('',_topBarChart.easyPercentage)],
        colorFn: (_ , __) => charts.Color.fromHex(code: "#BEFFEF"),
        labelAccessorFn: (TopBarData percent, _) =>
              (percent.percentage>0.1) ? '${((percent.percentage*100).round()).toString()}%':''
      ),
      charts.Series<TopBarData, String>(
        id: 'Medium',
        domainFn: (TopBarData percent, _) => percent._,
        measureFn: (TopBarData percent, _) => percent.percentage,
        data: [TopBarData('',_topBarChart.mediumPercentage )],
      ...
      charts.Series<TopBarData, String>(
        ...
        domainFn: (TopBarData percent, _) => percent._,
        measureFn: (TopBarData percent, _) => percent.percentage,
        data: [TopBarData('',_topBarChart.hardPercentage ),],
